...
class Tpq extends CI_Controller
{
    ...
    public function tagihan_siswa($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('admin/tagihan_siswa_tpq', $data);
            $this->load->view('templates/footer');
        }else{
            $this->Detail_model->tagihan_siswa_tpq();
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data sudah di-update!</div>');
            redirect('siswa/siswa_tpq');
        }
    }
